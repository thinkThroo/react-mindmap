...
function EventPlugin() {
    ...
    return {
        handleTopicClick: function (props) {
            // console.log("inside handleTopicClick", props);
        ...
        handleTopicDoubleClick: function (props) {
            // console.log("Double click editing a content node, here is the props::update`", props);
        ...
        handleActiveModalClose: function (props) {
            // console.log("aactive modal close?:props:", props);
            var controller = props.controller;
...
function OperationPlugin() {
    var startEditingProperties = function (_a) {
        var model = _a.model, topicKey = _a.topicKey;
        model = model.set('focusMode', 'EDIT_PROPERTIES')
            .set('focusKey', topicKey);
        window["TThroo"].editNode({
            model: model,
            topicKey: topicKey,
        });
        return model;
    ...
    var OpMap = new Map([
        ...
        [OpType.START_EDITING_CONTENT, startEditingProperties],
...
function TopicContent(props) {
    ...
    if (!readOnly) {
        console.log("readOnly is not a reliable technique", props);
        // if (window["TThroo"]) {
        //   console.log("window::", window["TThroo"].saveNode);
        //   window["TThroo"].saveNode(props);
        // }
    }
    ...
    function TopicContentWidget(props) {
        ...
        _this.onClick = function (ev) {
            ...
            _this.handleTopicClickTimeout = setTimeout(function () {
                ...
                // console.log("this is onClick inside topic-content-widget::", ev, props);
        ...
        _this.onClickCollapse = function (e) {
            e.stopPropagation();
            // console.log("what is onClickCollapse, this.props:", this.props);
...
var TopicHighlight = /** @class */ (function (_super) {
    ...
    TopicHighlight.prototype.render = function () {
        var saveRef = this.props.saveRef;
        return (createElement(FocusHighlightSvg, { ref: saveRef(RefKey.SVG_HIGHLIGHT_KEY) }, this.state.content));
    };
...
function renderDrawer(props) {
    var controller = props.controller, model = props.model, topicKey = props.topicKey;
    console.log("model.focusMode:::::_{}{{{", model.focusMode);
...
function RightTopPanel(props) {
    ...
    if (!expand) {
        return (createElement(PanelRoot, { zIndex: zIndex },
            createElement(IconBg, { onClick: function () { return setExpand(true); } },
                createElement(ShowMenuIcon, { className: iconClassName(IconName.SHOW_MENU) }))));
    }
    return (createElement(PanelRoot, { zIndex: zIndex },
        createElement(Title, null,
            createElement(CloseIcon, { className: iconClassName(IconName.CLOSE), onClick: function () { return setExpand(false); } })),
        createElement(StyledTabs, { id: selectedTabId, onChange: handleTabIdChange }, controller.run('renderRightTopPanelTabs', props))));
}
...
function TopicThemeEditor(props) {
    ...
    return (createElement(TopicThemeEditorRoot, null,
        ...
        createElement(SettingGroup, null,
            createElement(SettingTitle, null, "Background"),
            createElement(SettingItemColorPicker, { color: contentStyle.background, handleColorChange: function (color) {
                    setContentStyle({ background: color });
                } })),
        createElement(LinkStyleEditor, __assign({}, linkStyleEditorProps))));
}
...
var Diagram = /** @class */ (function (_super) {
    ...
    Diagram.prototype.openNewModel = function (newModel) {
        ...
        controller.run('operation', __assign(__assign({}, props), { opType: OpType.EXPAND_TO, topicKey: newModel.focusKey, model: newModel, callback: function () {
