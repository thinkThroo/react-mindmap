import React__default, { Component, createElement, useState, PureComponent, Fragment, useRef, useEffect } from 'react';
import ResizeObserver from 'resize-observer-polyfill';
import styled, { createGlobalStyle, ThemeProvider, css } from 'styled-components';
import debug from 'debug';
import { Colors, Checkbox, Popover, Button, NumericInput, InputGroup, MenuItem as MenuItem$1, Dialog, Classes, AnchorButton, Alert as Alert$1, Menu, Divider, ContextMenuTarget, Tabs, Tab, Tooltip, Position, Icon as Icon$2, PopoverPosition, Breadcrumb, Breadcrumbs, Drawer, Hotkey, Hotkeys, HotkeysTarget } from '@blueprintjs/core';
import { get, to } from 'color-string';
import { Select } from '@blueprintjs/select';
import { SketchPicker } from 'react-color';
import cx from 'classnames';
import { addEventListener } from 'consolidated-events';
import { DiagramLayoutType, OpType, DocModel, SheetModel, FocusMode, BlockType, ViewModeMindMap, getRelationship, TopicRelationship, DocModelModifier, isThemeType, TopicDirection, isAllSibiling, createKey, TopicVisualLevel, Controller } from '@blink-mind/core';
...
import { Tab as Tab$1, TabPanel, Tabs as Tabs$1, TabList } from '@slim-ui/react-tabs';
...
const DragScrollContent = styled.div `
  ...
  width: max-content;
`;
class DragScrollWidget extends Component {
    constructor(props) {
        super(props);
...
class SaveRef extends Component {
    constructor() {
        super(...arguments);
        this.getRef = name => {
            // log(this);
...
function setColorAlpha(color, alpha) {
    const [r, g, b, a] = get.rgb(color);
    return to.rgb([r, g, b, alpha]);
}
function setColorAlphaPercent(color, percent) {
    const [r, g, b, a] = get.rgb(color);
    return to.rgb([r, g, b, a * percent]);
}
...
const Title = styled.div `
  height: 16px;
`;
const TopicBlockIcon = styled.span `
  margin: 0px 10px;
  &:hover {
    color: palevioletred;
  }
...
const PanelTabRoot = styled.div `
  ...
  overflow: auto;
`;
const ToolbarItemPopoverTarget = styled.div `
  position: absolute;
  left: 0;
  top: 0;
  width: 100%;
  height: 100%;
`;
const VListContainer = styled.div `
...
const SettingItem = styled.span `
  margin: 0 10px 0 2px;
`;
const SettingBoxContainer = styled.div `
  padding: 10px;
  margin: 0 0 10px 0;
...
`;
const SettingLabel = styled(SettingItem) ``;
...
function SettingGroup(props) {
    return createElement("div", { className: "bm-setting-group bp3-divider" }, props.children);
}
function SettingItemFlex(props) {
    ...
    return (createElement(SettingItem, null,
        createElement(Flex, Object.assign({}, flexProps), children)));
}
function SettingItemColorPicker(props) {
    const { title, layout = 'h', color, handleColorChange } = props;
    return (createElement(SettingItemFlex, { layout: layout },
        title != null && createElement(Label, null, title),
        createElement(Popover, null,
            ...
            createElement("div", null,
                createElement(SketchPicker, { color: color, onChangeComplete: color => {
...
function SettingItemButton(props) {
    ...
    return (createElement(SettingItem, null,
        createElement(Button, Object.assign({}, restProps), title)));
}
function SettingItemNumericInput(props) {
    const { layout = 'h', labelWidth, title, ...restProps } = props;
    return (createElement(SettingItemFlex, { layout: layout },
        createElement(Label, { width: labelWidth }, title),
        createElement(NumericInput, Object.assign({}, restProps))));
}
function SettingItemInput(props) {
    const { layout = 'h', labelWidth, title, ...restProps } = props;
    return (createElement(SettingItemFlex, { layout: layout },
        createElement(Label, { width: labelWidth },
            title,
            " "),
        createElement(InputGroup, Object.assign({}, restProps))));
}
function SettingItemSelect(props) {
    ...
    return (createElement(SettingItemFlex, { layout: layout },
        title && createElement(Label, { width: labelWidth }, title),
        createElement(PxSelect, Object.assign({}, pxProps),
            createElement(Button, { text: text }))));
}
const renderItem = unit => (v, { handleClick }) => {
    return createElement(MenuItem$1, { text: `${v}${unit}`, key: v, onClick: handleClick });
};
const renderItemI18n = ctx => (v, { handleClick }) => {
    return createElement(MenuItem$1, { text: getI18nText(ctx, v), key: v, onClick: handleClick });
};
const PxSelect = Select.ofType();
...
function ToolbarItem(props) {
    ...
    return (createElement("div", Object.assign({}, nProps),
        createElement("div", { className: cx(iconClassName(iconName), iconCxName) }),
        children));
}
function ToolbarGroupItem(props) {
    const { items } = props;
    return (createElement("div", { className: "bm-toolbar-group" }, items.map(item => {
        const iProps = {
            ...item
        };
        return createElement(ToolbarItem, Object.assign({}, iProps));
    })));
}
...
function RenameDialog(props) {
    ...
    return (createElement(Dialog, { title: getI18nText(props, I18nKey.RENAME), onClose: handleClose, isOpen: isOpen },
        createElement("div", { className: Classes.DIALOG_BODY },
            createElement("input", { className: "bp3-input .modifier", type: "text", value: title, onChange: onChange, placeholder: "Text input", dir: "auto" })),
        createElement("div", { className: Classes.DIALOG_FOOTER },
            createElement("div", { className: Classes.DIALOG_FOOTER_ACTIONS },
                createElement(Button, { onClick: handleClose }, getI18nText(props, I18nKey.CANCEL)),
                createElement(AnchorButton, { onClick: handleConfirm(title) }, getI18nText(props, I18nKey.CONFIRM))))));
}
...
class OutsideClickHandler extends Component {
    ...
    render() {
        ...
        return (createElement("div", { ref: this.setChildNodeRef, style: display !== DISPLAY.BLOCK && objectValues(DISPLAY).includes(display)
                ? { display }
                : undefined }, children));
    }
}
...
function Alert(props) {
    ...
    return (createElement(Alert$1, Object.assign({}, alertProps),
...
function Toolbar(props) {
    ...
    return (createElement("div", { className: "bm-toolbar" }, controller.run('renderToolbarItems', props)));
}
...
function ToolbarItemLayout(props) {
    ...
    return (createElement(ToolbarItem, Object.assign({}, nProps),
        createElement(Popover, { enforceFocus: false },
            createElement(ToolbarItemPopoverTarget, null),
            createElement(Menu, null, layoutDirs.map(dir => (createElement(MenuItem$1, { key: dir[1], icon: Icon(dir[2]), text: dir[1], onClick: onClickSetLayout(dir[0]) })))))));
}
...
function ToolbarItemAddSheet(props) {
    ...
    return (createElement(MenuItem$1, { text: getI18nText(props, I18nKey.ADD_SHEET), onClick: onClick }));
}
...
function ToolbarItemMore(props) {
    ...
    return (createElement(ToolbarItem, Object.assign({ iconName: IconName.MORE, iconCxName: "more" }, props),
        createElement(Popover, { enforceFocus: false },
            createElement(ToolbarItemPopoverTarget, null),
            createElement(Menu, null, itemConfigs.map(item => createElement(item.element, {
                ...props,
...
const DIV = styled.div `
  ...
  height: 100%;
`;
class MindDragScrollWidget extends PureComponent {
    constructor(props) {
        super(props);
    ...
    render() {
        ...
        return (createElement(DIV, { onWheel: this.onWheel },
            createElement(DragScrollWidget, Object.assign({}, this.state, { enableMouseWheel: false, ref: saveRef(RefKey.DRAG_SCROLL_WIDGET_KEY + model.id) }), (setViewBoxScroll, setViewBoxScrollDelta) => {
                ...
                return (createElement(NodeLayer, { ref: saveRef(RefKey.NODE_LAYER + model.id) }, controller.run('renderRootWidget', rootWidgetProps)));
            })));
    }
}
...
function MindMapSheet_(props) {
    ...
    log$4('model', model);
    const [diagramState, setDiagramState_] = useState(controller.run('getInitialSheetState', props));
...
const Title$1 = styled.span `
  user-select: none;
`;
let SheetTitle = class SheetTitle extends Component {
    constructor(props) {
        super(props);
        this.onClickRename = () => {
    ...
    renderContextMenu() {
        ...
        return (createElement(Menu, null,
            createElement(MenuItem$1, { onClick: this.onClickRename, text: getI18nText(props, I18nKey.RENAME) }),
            createElement(Divider, null),
            createElement(MenuItem$1, { onClick: this.onClickDuplicate, text: getI18nText(props, I18nKey.DUPLICATE) }),
            createElement(MenuItem$1, { onClick: this.onClickDelete, text: getI18nText(props, I18nKey.DELETE) })));
    }
    render() {
        const props = this.props;
        const { controller } = props;
        ...
        return (createElement("div", null,
            createElement(Title$1, null, title),
            createElement(RenameDialog, Object.assign({}, renameDlgProps))));
    }
};
...
function RightTopPanelWidget(props) {
    ...
    return (createElement(PanelRoot, { zIndex: zIndex },
        createElement(Title, null,
            createElement(CloseIcon, { className: cx('bp3-button', iconClassName(IconName.CLOSE)), onClick: () => {
                    setRightTopPanelState({ isOpen: false });
                } })),
        createElement(StyledTabs, { id: "tabs", selectedTabId: selectedTabId, onChange: handleTabIdChange }, controller.run('renderRightTopPanelTabs', props))));
}

const SettingItemSelect$1 = props => (createElement(SettingItemSelect, Object.assign({ labelWidth: '45px' }, props)));
...
function BorderStyleEditor(props) {
    ...
    return (createElement(SettingGroup, null,
        createElement(SettingTitle, null, getI18nText(props, I18nKey.BORDER)),
        createElement(SettingRow, null,
            createElement(SettingItemSelect$1, { title: `${getI18nText(props, I18nKey.WIDTH)}:`, text: contentStyle.borderWidth
            ...
            createElement(SettingItemSelect$1, { title: `${getI18nText(props, I18nKey.STYLE)}:`, text: contentStyle.borderStyle
            ...
            createElement(SettingItemSelect$1, { title: `${getI18nText(props, I18nKey.RADIUS)}:`, text: contentStyle.borderRadius, items: borderRadiusItems, itemRenderer: renderItem('px'), onItemSelect: handleBorderRadiusChange }),
...
function ClearAllCustomStyle(props) {
    ...
    return (createElement(SettingGroup, null,
        createElement(SettingItem, null,
            createElement(Button, { onClick: handleClearAllCustomStyle }, getI18nText(props, I18nKey.CLEAR_ALL_CUSTOM_STYLE)))));
}
...
function LinkStyleEditor(props) {
    ...
    return (createElement(SettingGroup, null,
        createElement(SettingTitle, null, getI18nText(props, I18nKey.LINK)),
        showLinkStyle && (createElement(SettingRow, { alignItems: "center" },
            createElement(SettingRowTitle, null,
                getI18nText(props, I18nKey.LINK_TO_PARENT) + ':',
                ' '),
            createElement(SettingItemSelect, { text: `${getI18nText(props, I18nKey.WIDTH)}: ${linkStyle ? linkStyle.lineWidth : '0px'}`, items: borderWidthItems, itemRenderer: renderItem('px'), onItemSelect: handleLinkWidthChange }),
            createElement(SettingItemSelect, { text: `${getI18nText(props, I18nKey.LINE_TYPE)}: ${getI18nText(props, linkStyle.lineType)}`, items: lineTypes, itemRenderer: renderItemI18n(props), onItemSelect: handleLinkTypeChange }),
        ...
        showSubLinkStyle && (createElement(SettingRow, { alignItems: "center" },
            createElement(SettingRowTitle, null, getI18nText(props, I18nKey.SUB_LINKS) + ': '),
            createElement(SettingItemSelect, { text: `${getI18nText(props, I18nKey.WIDTH)}: ${subLinkStyle ? subLinkStyle.lineWidth : '0px'}`, items: borderWidthItems, itemRenderer: renderItem('px'), onItemSelect: handleSubLinkWidthChange }),
            createElement(SettingItemSelect, { text: `${getI18nText(props, I18nKey.LINE_TYPE)}: ${getI18nText(props, subLinkStyle.lineType)}`, items: lineTypes, itemRenderer: renderItemI18n(props), onItemSelect: handleSubLinkTypeChange }),
...
function StyleEditor(props) {
    ...
    return (createElement(PanelTabRoot, null,
        viewModeMindMap ? (createElement(BorderStyleEditor, Object.assign({}, contentStyleEditorPros))) : null,
        controller.run('renderTextStyleEditor', contentStyleEditorPros),
        viewModeMindMap ? (createElement(SettingGroup, null,
            createElement(SettingTitle, null, getI18nText(props, I18nKey.BACKGROUND)),
            createElement(SettingItemColorPicker, { color: contentStyle.background, handleColorChange: handleBackgroundColorChange }))) : null,
        viewModeMindMap ? createElement(LinkStyleEditor, Object.assign({}, linkStyleEditorProps)) : null,
        createElement(SettingGroup, null,
            createElement(SettingItemButton, { title: getI18nText(props, I18nKey.CLEAR_TOPIC_STYLE), onClick: handleClearStyle }),
            createElement(SettingItemButton, { title: getI18nText(props, I18nKey.COPY_STYLE), onClick: handleCopyStyle }),
            createElement(SettingItemButton, { title: getI18nText(props, I18nKey.PASTE_STYLE), onClick: handlePasteStyle })),
...
function PaddingStyleEditor(props) {
    ...
    return (createElement(SettingGroup, null,
        createElement(SettingTitle, null, getI18nText(props, I18nKey.PADDING)),
        createElement(SettingRow, null,
            createElement(SettingItemNumericInput, Object.assign({}, topProps)),
            createElement(SettingItemNumericInput, Object.assign({}, rightProps)),
            createElement(SettingItemNumericInput, Object.assign({}, bottomProps)),
            createElement(SettingItemNumericInput, Object.assign({}, leftProps)))));
}

const SettingItemSelect$2 = props => (createElement(SettingItemSelect, Object.assign({ labelWidth: '80px' }, props)));
function TextStyleEditor(props) {
    ...
    const fontFamilyProps = {
        ...
        text: createElement("span", { style: { fontFamily: fontF } }, fontF),
        items: controller.run('getFontList', props),
        itemRenderer: item => {
            const text = createElement("span", { style: { fontFamily: item } }, item);
            return (createElement(MenuItem$1, { key: item, text: text, onClick: () => {
    ...
    const fontStyleProps = {
        ...
        itemRenderer: (v, { handleClick }) => {
            return createElement(MenuItem$1, { key: v, text: v, onClick: handleClick });
        },
        onItemSelect: item => {
            setContentStyle({
                fontStyle: item
    ...
    const fontWeightProps = {
        ...
        itemRenderer: (v, { handleClick }) => {
            return (createElement(MenuItem$1, { key: v, text: fontWeightItemsMap.get(v), onClick: handleClick }));
        },
        onItemSelect: item => {
            setContentStyle({
                fontWeight: item
    ...
    return (createElement(SettingGroup, null,
        createElement(SettingTitle, null, getI18nText(props, I18nKey.TEXT_EDITOR)),
        createElement(SettingRow, null,
            createElement(SettingItemSelect$2, Object.assign({}, fontFamilyProps)),
            createElement(SettingItemNumericInput, Object.assign({}, fontSizeNumInputProps))),
        createElement(SettingRow, null,
            createElement(SettingItemSelect$2, Object.assign({}, fontWeightProps)),
            createElement(SettingItemSelect$2, Object.assign({}, fontStyleProps))),
        createElement(SettingRow, null,
            createElement(SettingItemInput, Object.assign({}, lineHeightInputProps)),
...
function TopicThemeEditor(props) {
    ...
    return (createElement(TopicThemeEditorRoot, null,
        createElement(BorderStyleEditor, Object.assign({}, nProps)),
        createElement(PaddingStyleEditor, Object.assign({}, nProps)),
        createElement(TextStyleEditor, Object.assign({}, nProps)),
        createElement(SettingGroup, null,
            createElement(SettingTitle, null, getI18nText(props, I18nKey.BACKGROUND)),
            createElement(SettingItemColorPicker, { color: contentStyle.background, handleColorChange: color => {
                    setContentStyle({ background: color });
                } })),
...
function ThemeEditor(props) {
    ...
    // 注意这里一定要用cloneDeep
    const theme = cloneDeep(model.config.theme);
    ...
    const rootTopicEditor = createElement(TopicThemeEditor, Object.assign({}, rootTopicEditorProps));
    const primaryTopicEditorProps = {
        ...props,
        topicStyle: primaryTopic,
        setTopicStyle: setPrimaryTopicStyle
    };
    const primaryTopicEditor = createElement(TopicThemeEditor, Object.assign({}, primaryTopicEditorProps));
    const normalTopicEditorProps = {
        ...props,
        topicStyle: normalTopic,
        setTopicStyle: setNormalTopicStyle
    };
    const normalTopicEditor = createElement(TopicThemeEditor, Object.assign({}, normalTopicEditorProps));
    const tabsProps = {
        id: tabId,
    ...
    const topicThemes = (createElement(SettingBoxContainer, null,
        createElement(Tabs, Object.assign({}, tabsProps),
            createElement(Tab, { id: "normal", title: getI18nText(props, I18nKey.NORMAL_TOPIC), panel: normalTopicEditor }),
            createElement(Tab, { id: "primary", title: getI18nText(props, I18nKey.PRIMARY_TOPIC), panel: primaryTopicEditor }),
            createElement(Tab, { id: "root", title: getI18nText(props, I18nKey.ROOT_TOPIC), panel: rootTopicEditor }))));
    ...
    const alert = createElement(Alert, Object.assign({}, alertProps));
    return (createElement(PanelTabRoot, null,
        createElement(SettingGroup, null,
            createElement(SettingTitle, null, getI18nText(props, I18nKey.GLOBAL)),
            createElement(SettingRow, null,
                createElement(SettingLabel, null, getI18nText(props, I18nKey.BACKGROUND) + ':'),
                createElement(SettingItemColorPicker, { color: theme.background, handleColorChange: handleBackgroundColorChange }),
                createElement(SettingLabel, null, getI18nText(props, I18nKey.HIGHLIGHT) + ':'),
                createElement(SettingItemColorPicker, { color: theme.highlightColor, handleColorChange: handleHighlightColorChange }),
                createElement(SettingLabel, null, getI18nText(props, I18nKey.RANDOM_COLOR) + ':'),
                createElement(StyledCheckbox, { checked: theme.randomColor, onChange: handleRandomColorChange }))),
        topicThemes,
        createElement(SettingGroup, null,
            createElement(SettingItemButton, { title: getI18nText(props, I18nKey.EXPORT_THEME), onClick: handleExportTheme }),
            createElement(SettingItemButton, { title: getI18nText(props, I18nKey.IMPORT_THEME), onClick: handleImportTheme })),
        alert));
}
...
const Item = props => {
    return (createElement(Tooltip, { content: props.tooltip, position: Position.TOP, className: Classes.ICON },
        createElement("div", { className: "bm-btn", onClick: props.onClick, tabIndex: -1 }, props.children)));
};
class ViewPortViewer extends BaseWidget {
    ...
    render() {
        ...
        return (createElement(ViewerRoot, { zIndex: zIndex },
            createElement(Item, { onClick: this.onClickCollapseAll, tooltip: getI18nText(props, I18nKey.COLLAPSE_ALL) }, Icon(IconName.COLLAPSE_ALL)),
            createElement(Item, { onClick: this.onClickExpandAll, tooltip: getI18nText(props, I18nKey.EXPAND_ALL) }, Icon(IconName.EXPAND_ALL)),
            model.config.viewMode === ViewModeMindMap ? (createElement(Item, { onClick: this.centerRootTopic, tooltip: getI18nText(props, I18nKey.CENTER_ROOT_TOPIC) }, Icon(IconName.CENTER))) : null,
            createElement(Item, { onClick: this.onClickMinusZoom, tooltip: getI18nText(props, I18nKey.ZOOM_IN) }, Icon(IconName.MINUS)),
            createElement(Item, { onClick: this.onClickResetZoom, tooltip: getI18nText(props, I18nKey.RESET) },
                createElement("span", { className: "iconfont" }, `${getI18nText(props, I18nKey.ZOOM)}:${Math.floor(zoomFactor * 100)}%`)),
            createElement(Item, { onClick: this.onClickAddZoom, tooltip: getI18nText(props, I18nKey.ZOOM_OUT) }, Icon(IconName.PLUS))));
    }
}
...
function DiagramRoot(props) {
    ...
    const tabs = sheetModels.map(model => {
        ...
        return (createElement(Tab$1, Object.assign({ key: index }, tabProps),
            createElement(SheetTitle, Object.assign({}, nProps))));
    });
    tabs.push(createElement(Tab$1, Object.assign({ key: i++ }, tabProps),
        createElement(Icon$2, { onClick: onClickAddSheet, icon: IconNames.PLUS })));
    i = 0;
    // const tabPanelProps = {
    ...
    const tabPanels = sheetModels.map(model => {
        return (createElement(TabPanel, Object.assign({ key: i++ }, tabPanelProps),
            createElement(Theme, { theme: model.config.theme }, controller.run('renderSheet', {
                ...props,
                model
            }))));
    });
    tabPanels.push(createElement(TabPanel, Object.assign({ key: i++ }, tabPanelProps)));
    const tabsProps = {
    ...
    const child = (createElement(TabsContainer, null,
        createElement(Tabs$1, Object.assign({}, tabsProps),
            tabPanels,
            createElement(TabList, { className: "tab-list" }, tabs))));
    return (createElement(Root, { onCopy: onCopy },
...
function DialogWidget(props) {
    ...
    return (createElement(Dialog, Object.assign({}, _dialogProps),
        createElement("div", { className: Classes.DIALOG_BODY, style: { minHeight: 0 } }, dialogContent)));
}
...
class EditorRootBreadcrumbs extends BaseWidget {
    constructor(props) {
        ...
        this.breadcrumbRenderer = props => {
            ...
            //TODO
            const content = createElement(TooltipContent, null, text);
            const tooltipProps = {
                content,
                position: PopoverPosition.BOTTOM_RIGHT
            };
            const breadcrumb = (createElement(Breadcrumb, Object.assign({}, breadProps),
...
class RootWidget extends Component {
    ...
    render() {
        ...
        const rootTopic = (createElement(Topic, { ref: saveRef(topicNodeRefKey(topicKey)) }, topicContent));
...
function TopicDesc(props) {
    ...
    return (createElement(TopicBlockIcon, { onClick: onClick, className: iconClassName(IconName.NOTES), tabIndex: -1 }));
}
...
class TopicDropEffect extends BaseWidget {
    ...
    render() {
        ...
        return (createElement(DropEffectSvg, { ref: saveRef(RefKey.SVG_DROP_EFFECT_KEY + model.id) }, this.state.content));
    }
}
...
let TopicNodeWidget = class TopicNodeWidget extends BaseWidget {
    ...
    render() {
        ...
        return (createElement(TopicNodeRoot, { ref: saveRef(topicNodeRefKey(topicKey)) },
            prevDropArea,
            createElement(Column, null,
                createElement(NodeRows, Object.assign({}, nodeProps), controller.run('renderTopicNodeRows', props)),
...
function TopicNodeLastRow(props) {
    ...
    return (createElement(TopicNodeRow, { ref: ref },
...
class TopicSubLinks extends BaseWidget {
    ...
    layout() {
        ...
        topic.subKeys.forEach(key => {
            ...
            const x = (dir === TopicDirection.RIGHT ? rect.left : rect.right) - svgRect.left;
            p3 = { x, y };
            p4 = linkStyle.hasUnderline
                ? {
                    x: (dir === TopicDirection.RIGHT ? rect.right : rect.left) -
                        svgRect.left +
...
const TopicWidgetRoot = styled.div `
  display: flex;
  align-items: stretch;
  flex-direction: ${props => props.topicDirection === TopicDirection.RIGHT ? 'row' : 'row-reverse'};
`;
// TODO
const NodeChildren = styled.div `
  position: relative;
  padding: ${props => props.dir === TopicDirection.RIGHT
...
const SubNodes = styled.div `
  ...
  height: 100%;
`;
class TopicWidget extends Component {
    renderSubTopics(props) {
        ...
        return (createElement(NodeChildren, { dir: dir, marginH: model.config.theme.marginH },
            createElement(SubNodes, null, subTopics),
            subLinks));
    }
    ...
    render() {
        ...
        return (createElement(TopicWidgetRoot, { topicDirection: dir, ref: saveRef(topicWidgetRootRefKey(topicKey)) },
            topicNode,
        ...
        customizeTopicContextMenu(ctx) {
            ...
            return items.map(item => {
                ...
                if (item.viewMode && !item.viewMode.includes(viewMode))
                    return null;
                return (createElement(MenuItem$1, { key: item.label, icon: Icon(item.icon), text: getI18nText(ctx, item.label), labelElement: createElement(KeyboardHotKeyWidget, { hotkeys: item.shortcut }), onClick: onClickItem(item) }));
            });
        }
    };
}
...
function DragAndDropPlugin() {
    ...
    return {
        ...
        renderDragAndDropEffect(props) {
            ...
            return (createElement(TopicDropEffect, Object.assign({ ref: saveRef(RefKey.DROP_EFFECT_KEY + model.id) }, props)));
        },
...
function LinkLinePlugin() {
    return {
        renderSubLinks(props) {
            ...
            if (topic.subKeys.size === 0 || topic.collapse)
                return null;
            return createElement(TopicSubLinks, Object.assign({ ref: saveRef(linksRefKey(topicKey)) }, props));
        },
        renderRootSubLinks(props) {
            ...
            return createElement(RootSubLinks, Object.assign({ ref: saveRef(linksRefKey(topicKey)) }, props));
        }
...
function renderDrawer(props) {
    ...
    if (model.focusMode === FocusMode.EDITING_DESC) {
        ...
        return (createElement(Drawer, { key: "drawer", title: getI18nText(props, I18nKey.EDIT_NOTES), icon: Icon('note'), isOpen: true, hasBackdrop: true, backdropProps: { onMouseDown: stopPropagation }, isCloseButtonShown: false, onClose: onDescEditorClose, size: "70%" }, descEditor));
    }
}
...
function RightTopPanelPlugin() {
    return {
        ...
        renderTextStyleEditor(ctx) {
            return createElement(TextStyleEditor, Object.assign({}, ctx));
        },
...
function ToolbarPlugin() {
    return {
        ...
        renderToolbarItems(props) {
            ...
            itemConfigs.sort((a, b) => a.order - b.order);
            log$j(itemConfigs);
            return itemConfigs.map(item => createElement(item.element, {
                ...props,
                key: item.order.toString()
            }));
        },
...
function TopicContentEditor(props) {
    ...
    const innerEditorDivRef = props.innerEditorDivRef || useRef();
    const handleKeyDown = e => {
...
let Diagram = class Diagram extends Component {
    constructor() {
        super(...arguments);
    ...
    renderHotkeys() {
        ...
        if (model.focusMode === FocusMode.NORMAL ||
            ...
            hotKeys.topicHotKeys.forEach((v, k) => {
                children.push(createElement(Hotkey, Object.assign({ key: k }, v, { global: true })));
            });
...
export { Alert, BaseWidget, Btn, COLORS, CloseIcon, ColorBar, DefaultPlugin, Diagram, DragScrollWidget, EventKey, Flex, GlobalStyle, HotKeyName, I18nKey, Icon, IconBg, IconName, InlineBlockSpan, KeyboardHotKeyWidget, Margin, MoveTopicDir, OutsideClickHandler, PanelTabRoot, PropKey, PxSelect, RefKey, RenameDialog, SaveRef, SettingBoxContainer, SettingGroup, SettingItem, SettingItemButton, SettingItemColorPicker, SettingItemFlex, SettingItemInput, SettingItemNumericInput, SettingItemSelect, SettingLabel, SettingRow, SettingRowTitle, SettingTitle, ShowMenuIcon, StyledCheckbox, TempValueKey, Title, Toolbar, ToolbarGroupItem, ToolbarItem, ToolbarItemAddSheet, ToolbarItemLayout, ToolbarItemMore, ToolbarItemPopoverTarget, TopicBlockIcon, VListContainer, WithBorder, ZIndex, borderWidthItems, browserDownloadFile, browserDownloadText, browserOpenFile, center, centerPointX, centerPointY, centerX, centerY, collapseRefKey, contentEditorRefKey, contentRefKey, descEditorRefKey, dropAreaRefKey, getComputedStyle, getI18nText, getLinkKey, getRelativeRect, getRelativeVector, iconClassName, isOSX, linksRefKey, linksSvgRefKey, op, paddingCss, renderItem, renderItemI18n, selectTo, selectToEnd, selectToStart, setColorAlpha, setColorAlphaPercent, stopPropagation, swap, topicNodeRefKey, topicWidgetRefKey, topicWidgetRootRefKey, tvZoomFactorKey, useClickOutside, useEventListener };
